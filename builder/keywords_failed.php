...
<?php
$select = mysql_query("SELECT idkeywords FROM `keywords` WHERE `reason`!=''") or die(mysql_error());
...
$select1 = mysql_query("SELECT idkeywords,original,reason FROM `keywords` WHERE `reason`!='' ORDER BY `idkeywords` DESC LIMIT ".$first.",".$last."") or die(mysql_error());
...
if($page > 1)
	{
		echo "&nbsp;<a href='keywords_failed.php?page=1'>First</a>";
	}
	...
	for ($i = ($page - 2); $i <= ($page + 2); $i++)
	{
		if($i > 0 && $i <= $seiten)
		{
			...
			{
			echo "&nbsp;-&nbsp;<a href=keywords_failed.php?page=".$i.">";
				echo "".$i."";
			}
			echo "</a>";
		}
	...
	{
		echo "&nbsp;-&nbsp;<a href=keywords_failed.php?page=".$seiten.">Last</a>";
	}
