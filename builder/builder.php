	...
	while($row = mysqli_fetch_object($select)){
		...
		if(mysqli_num_rows($qasel) > 0){
			while($qa = mysqli_fetch_object($qasel)){
				...
				$content = str_replace('%qa'.$i.'%', "<div style=\"border: 2px solid #eee;padding:20px 10px 20px 10px;\"><p><h2 id=\"".$i."\">".$qa->question."</h2>".$qa->answer_en."</p></div><br>", $content);
				//put all questions and answers
				$qas[] = "<div style=\"border: 2px solid #eee;padding:20px 10px 20px 10px;\"><p><h2 id=\"".$i."\">".$qa->question."</h2>".$qa->answer_en."</p></div>";
				$i++;
			}
		}
