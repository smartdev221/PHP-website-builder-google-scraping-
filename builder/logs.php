...
function new_window(x, y) {
  var myWindow = window.open("log.php?id="+x+"&type="+y, "info", "width=700,height=700");
}
</script>
...
<?php
$select = mysql_query("SELECT COUNT(`id`) as total FROM `logs` ORDER BY id DESC") or die(mysql_error());
$total_entries = mysql_fetch_object($select)->total;
...
$select1 = mysql_query("SELECT * FROM `logs` ORDER BY id DESC LIMIT ".$first.",".$last."") or die(mysql_error());
...
if($page > 1)
	{
		echo "&nbsp;<a href='logs.php?page=1'>First</a>";
	}
	...
	{
		echo "&nbsp;-&nbsp;<a href=logs.php?page=".$seiten.">Last</a>";
	}
