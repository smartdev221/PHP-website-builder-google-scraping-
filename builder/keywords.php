            ...
            <div class="row">
                <div class="span12">
                    ...
                    <div class="widget_container">
                        <a href="keywords_scraped.php" class="btn btn-medium btn-primary">Show just scraped</a>
                        <a href="keywords_built.php" class="btn btn-medium btn-primary">Show just built</a>
                        <a href="keywords_failed.php" class="btn btn-medium btn-danger">Show failed</a>
                        <a href="keywords_failed_serp.php" class="btn btn-medium btn-danger">Failed serp filter</a>
                        <?php
if(!empty($_GET['delete'])){
$delete = mysql_query("DELETE FROM `keywords` WHERE `idkeywords`='".mysql_real_escape_string($_GET['delete'])."'");
if($delete){
...
if(isset($_POST['submit'])){
	...
	foreach($_POST['topic'] as $topic){
		if($i > 0){
			$todel .= " or `idkeywords`='".$topic."'";
		} else {
			$todel .= "`idkeywords`='".$topic."'";
		}
		$i++;
	}
		//echo $todel;
			$del = mysql_query("DELETE FROM `keywords` WHERE ".$todel) or die(mysql_error());
		if($del){
			echo '<span style="background-color: rgb(223, 240, 216);">The selected keywords were deleted.</span>';
		} else {
			echo '<span style="background-color: rgb(242, 222, 222);">The selected keywords were not deleted. Try again!</span>';
		}
	} else {
		echo '<span style="background-color: rgb(242, 222, 222);">You did not selected any keyword to be deleted!</span>';
	}
}
?>
                        ...
                        <form action="#" method="post">
                            <table class="table table-hover">
                                ...
                                <?php
$select = mysql_query("SELECT idkeywords FROM `keywords` ORDER BY scraped DESC") or die(mysql_error());
...
$select1 = mysql_query("SELECT idkeywords,original,scraped,video,images,url_scraped,result1 FROM `keywords` ORDER BY scraped DESC LIMIT ".$first.",".$last."") or die(mysql_error());
while($row = mysql_fetch_object($select1)){

if($row->scraped == '1'){
	$class = ' style="background-color: rgb(223, 240, 216);"';
} elseif($row->scraped == '2') {
...
?>
                                <tr<?php echo $class; ?>>
                                    ...
                                    <td><?php if($row->result1 == "") { echo yesno(0,$row->idkeywords, 1); } else { echo yesno($row->scraped,$row->idkeywords, 1); } ?>
                                    </td>
                                    <td><?php echo yesno($row->video,$row->idkeywords, 2); ?></td>
                                    <td><?php echo yesno($row->images,$row->idkeywords, 3); ?></td>
                                    <td><?php echo yesno($row->url_scraped,$row->idkeywords, 4); ?></td>
                                    <td><a href="keywords.php?delete=<?php echo $row->idkeywords; ?>">Delete</td>
                                    </tr>
                                    <?php
}
...
if($page > 1)
	{
		echo "&nbsp;<a href='keywords.php?page=1'>First</a>";
	}
	...
	for ($i = ($page - 2); $i <= ($page + 2); $i++)
	{
		if($i > 0 && $i <= $seiten)
		{
			...
			{
			echo "&nbsp;-&nbsp;<a href=keywords.php?page=".$i.">";
				echo "".$i."";
			}
			echo "</a>";
		}
	...
	{
		echo "&nbsp;-&nbsp;<a href=keywords.php?page=".$seiten.">Last</a>";
	}
