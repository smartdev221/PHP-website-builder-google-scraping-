...
<?php
$select = mysql_query("SELECT idkeywords FROM `keywords` WHERE `built`='1'") or die(mysql_error());
...
$select1 = mysql_query("SELECT idkeywords,original,scraped,video,images,url_scraped,translated_qa,translated_snippet,built FROM `keywords` WHERE `built`='1' order by `idkeywords` DESC LIMIT ".$first.",".$last."") or die(mysql_error());
...
?>		
		<tr<?php echo $class; ?>>
			...
			<td><?php echo yesno($row->built,$row->idkeywords, 7); ?></td>
...
if($page > 1)
	{
		echo "&nbsp;<a href='keywords_built.php?page=1'>First</a>";
	}
	...
	for ($i = ($page - 2); $i <= ($page + 2); $i++)
	{
		if($i > 0 && $i <= $seiten)
		{
			...
			{
			echo "&nbsp;-&nbsp;<a href=keywords_built.php?page=".$i.">";
				echo "".$i."";
			}
			echo "</a>";
		}
	...
	{
		echo "&nbsp;-&nbsp;<a href=keywords_built.php?page=".$seiten.">Last</a>";
	}
