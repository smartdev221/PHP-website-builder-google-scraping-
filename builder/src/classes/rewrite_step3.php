...
function __construct($link){
	$this->link = $link;
	echo "inside rewrite_step3 constructor<br>\n";
	...
	$sel = mysqli_query($this->link, "SELECT * FROM `rewrite_steps` WHERE `step2_completed`='1' and `step3_completed`='0' LIMIT 1");
	
	while($row = mysqli_fetch_object($sel)){
		
		mysqli_query($this->link, "UPDATE `rewrite_steps` SET `step3_completed`='2' WHERE `idkeywords`='".$row->idkeywords."'");
		
		logger($row->idkeywords, $row->original, 'Rewrite steps... 3', $this->link);
		...
		$serp_content = $row->step2_text_final;
		$original_text = $row->original_text;

		
		$step_3_setting = explode('|', $result->rewrite_step_3);
		$step_3 = $step_3_setting[0];
		$step_3_options = $step_3_setting[1];
		$step_3_langs = explode(';', $step_3_options);
		
		//if 3rd step is Wordtune
		if($step_3 == 1){
			if($step_3_options == 1){
			...
			} elseif($step_3_options == 2){
				//first sentence per paragraph
				preg_match_all('/<p>(.*[.?!:])\s?/U', $serp_content, $matches);
				foreach($matches[1] as $match){
					$insert = mysqli_query($this->link, "INSERT INTO `rewrite_wordtune`(`idkeywords`, `content`, `step`) VALUES('".$row->idkeywords."', '".mysqli_real_escape_string($this->link, $match)."', '3')");
					$serp_content = str_replace($match, '[#'.mysqli_insert_id($this->link).'#]', $serp_content);
					echo $match."<br>";
				}
			} elseif($step_3_options == 3){
				//just headings
			...
			} elseif($step_3_options == 6){
				//"Just snippets/PAA"
				//complicated, left out for now
			}
			
			mysqli_query($this->link, "UPDATE `rewrite_steps` SET `step3_text`='".mysqli_real_escape_string($this->link, $serp_content)."', `wordtune`='2' WHERE `idkeywords`='".$row->idkeywords."'");
				
		} elseif($step_3 == 2){
			//if 3rd step is Spintax
			$text_final = $this->checkspin($serp_content, $step_3_options);
			
			$differences = Diff::compare($original_text, $text_final);
			$difference = 100-(Diff::toTable1($differences[0], $differences[1]));		
			
			mysqli_query($this->link, "UPDATE `rewrite_steps` SET `step3_text`='".mysqli_real_escape_string($this->link, $serp_content)."', `step3_text_final`='".mysqli_real_escape_string($this->link, $text_final)."', `step3_uniqueness`='".$difference."', `step3_completed`='1' WHERE `idkeywords` = '".$row->idkeywords."'");
		} elseif($step_3 == 3){
			//if 3rd step is GTR
			mysqli_query($this->link, "UPDATE `rewrite_steps` SET `step3_text`='".mysqli_real_escape_string($this->link, $serp_content)."' WHERE `idkeywords` = '".$row->idkeywords."'");
			$insert = mysqli_query($this->link, "INSERT INTO `rewrite_translate`(`idkeywords`, `feed_text`, `lang1_code`, `lang2_code`, `lang3_code`, `step`) VALUES('".$row->idkeywords."', '".mysqli_real_escape_string($this->link, $serp_content)."', '".$step_3_langs[0]."', '".$step_3_langs[1]."', '".$step_3_langs[2]."', '3')");
