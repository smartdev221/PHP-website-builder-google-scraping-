...
$select = mysqli_query($this->link, "SELECT * FROM `config_global`") or die(mysqli_error($this->link));
	...
	$sel = mysqli_query($this->link, "SELECT * FROM `keywords` where `result1`!='' and `scraped`='1' and `serp_checked`='0' and `built`='0' LIMIT 50");
	//$sel = mysqli_query($this->link, "SELECT * FROM `keywords` where `result1`!='' and `scraped`='1' and `idkeywords`='1'");
	
	while($row = mysqli_fetch_object($sel)){
		logger($row->idkeywords, $row->original, 'Serp filtering', $this->link);
		echo "<hr>keyword id:".$row->idkeywords."<br>";
		$positive_match = 0;
		$negative_match = 0;
		if(empty($serp_positive) && empty($serp_negative)){
			mysqli_query($this->link, "UPDATE `keywords` SET `serp_checked`='1' WHERE `idkeywords`='".$row->idkeywords."'");
			echo "No positive and negative serp matches added.<br>";
			logger($row->idkeywords, $row->original, 'Serp filtering: No positive or negative filters added -> pass.', $this->link);
		} else {
			...
			if($positive_match == 1 && $negative_match == 0){
				logger($row->idkeywords, $row->original, 'Serp filtering: pass.', $this->link);
				mysqli_query($this->link, "UPDATE `keywords` SET `serp_checked`='1',`last_action`='serp_filter', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
				$result = "passed";
			}elseif($negative_match == 1){
				logger($row->idkeywords, $row->original, 'Serp filtering: not passed (Negative serp match filter for '.$instances1[0].' with a required count of '.$instances1[1].', actual match is '.count($matches1[0]).').', $this->link);
				mysqli_query($this->link, "UPDATE `keywords` SET `serp_checked`='1',`built`='4',`reason`='Negative serp match filter for ".$instances1[0]." with a required count of ".$instances1[1].", actual match is ".count($matches1[0])."' WHERE `idkeywords`='".$row->idkeywords."'") or die(mysqli_error($this->link));
				...
				logger($row->idkeywords, $row->original, 'Serp filtering: passed (No match for positive or negative serp filters)', $this->link);
				//mysqli_query($this->link, "UPDATE `keywords` SET `serp_checked`='1',`built`='4',`reason`='No match for positive or negative serp filters.' WHERE `idkeywords`='".$row->idkeywords."'");
				mysqli_query($this->link, "UPDATE `keywords` SET `serp_checked`='1' WHERE `idkeywords`='".$row->idkeywords."'");
				//mysqli_query($this->link, "UPDATE `websites` SET `scraped`=`scraped`-1,`scraped_img`=`scraped_img`-1 WHERE `id`='".$row->website."'");
				$result = "passed";
			}
