...
function __construct($link){
	$this->link = $link;
	echo "inside builder constructor<br>\n";
	...
	while($website = mysqli_fetch_object($get_templates)){
		for($i = 1; $i < 6; $i++){
			...
			$find_and_replace[$website->id] = $website->find_and_replace;
			$box_shadow[$website->id] = $website->paa_shadow;
			$preload[$website->id] = $website->preload;
			$total_built[$website->id] = $website->total_built;
			$exclude_paa_title[$website->id] = $website->exclude_paa_title;
		}
	}
	...
	///check if it built under set number
		$website_check = mysqli_query($this->link, "SELECT * FROM `websites` WHERE `built`<`dripfeed_current` and `skip`!='1' order by rand() LIMIT 1");
		if(mysqli_num_rows($website_check) > 0){
			$website_id = mysqli_fetch_object($website_check);
			
	$select = mysqli_query($this->link, "SELECT * FROM `keywords` WHERE `scraped`='1' and `images`='1' and `url_scraped`='1' and `translated_qa`='1' and `translated_snippet`='1' and `translated_serp`='1' and `translated_serp_title`='1' and `translated_headline`='1' and `built`='0' and `website`='".$website_id->id."' LIMIT 1");//5//1//3
	
	while($row = mysqli_fetch_object($select)){
		
			logger($row->idkeywords, $row->original, 'Build start', $this->link);
		...
		//serp content 
		$serpsel = mysqli_query($this->link, "SELECT * FROM `scraped_content_serp` WHERE `idkeywords`='".$row->idkeywords."'");
		...
		//headline text
		$headlinesel = mysqli_query($this->link, "SELECT * FROM `headlines` WHERE `idkeywords`='".$row->idkeywords."'");
		...
		if($pref[$row->website] == 1){
			$ptitle = $this->prefixes($ptitle);
		}
		//use serp titles instead of text boxes
		if($serpt[$row->website] == 1){
			$get_title = mysqli_query($this->link, "SELECT * FROM `serp_titles` WHERE `idkeywords`='".$row->idkeywords."' and `translated`='1'");
		...
		//intro text
		$introsel = mysqli_query($this->link, "SELECT * FROM `intros` WHERE `idkeywords`='".$row->idkeywords."'");
		...
		$images = array();
		for($i = 1; $i < 21; $i++){
			$check_blacklist = $this->ignore_url($row->{'imgresult'.$i});
		...
		foreach($images as $image){
			if($i == 1){
				...
				$filename = $this->download($image, url_slug($row->original), $ftp[$row->website], $ftpuser[$row->website], $ftppassword[$row->website], $row->website);
		...
		$qasel = mysqli_query($this->link, "SELECT * FROM `qa` WHERE `original`='".mysqli_real_escape_string($this->link, $row->original)."' LIMIT 10");
		$qas = array();
		$i = 1;
		if(mysqli_num_rows($qasel) > 0){
			$box_shadow_style = $box_shadow[$website_id->id];
			while($qa = mysqli_fetch_object($qasel)){
				...
				if($exclude_paa_title[$row->website] == 1){
					$content = str_replace('%qa'.$i.'%', "<div style=\"".$box_shadows[$box_shadow_style]."padding:20px 10px 20px 10px;\"><p id=\"".$i."\">".$qa->answer_en."</p></div><br>", $content);
					//put all questions and answers
					$qas[] = "<div style=\"".$box_shadows[$box_shadow_style]."padding:20px 10px 20px 10px;\"><p id=\"".$i."\">".$qa->answer_en."</p></div><br>";
				} else {
					$content = str_replace('%qa'.$i.'%', "<div style=\"".$box_shadows[$box_shadow_style]."padding:20px 10px 20px 10px;\"><p><h2 id=\"".$i."\">".$qa->question."</h2>".$qa->answer_en."</p></div><br>", $content);
					//put all questions and answers
					$qas[] = "<div style=\"".$box_shadows[$box_shadow_style]."padding:20px 10px 20px 10px;\"><p><h2 id=\"".$i."\">".$qa->question."</h2>".$qa->answer_en."</p></div><br>";
				}
				$i++;
			}
		}
		...
		//featuredsnippets
		$fnsel = mysqli_query($this->link, "SELECT * FROM `featuredsnippets` WHERE `original`='".mysqli_real_escape_string($this->link, $row->original)."' LIMIT 10");
		$snippets = array();
		$i = 0;
		$nosnippets = 0;
		...
		$posted = "";
		$biobox = "";
		$authorssel = mysqli_query($this->link, "SELECT * FROM `authors` WHERE `website`='".$row->website."' order by RAND() LIMIT 1");
	  ...
	  //clear the tokens that were not fulfilled
		...
		//find_and_replace
		$content = $this->find_and_replace($content, $find_and_replace[$row->website]);
		...
		if($wsnippets[$row->website] == 0 && $nosnippets == 1){
			...
			mysqli_query($this->link, "UPDATE `keywords` SET `built`='3',`reason`='No snippets.',`last_action`='builder', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
			mysqli_query($this->link, "UPDATE `websites` SET `scraped`=`scraped`-1,`scraped_img`=`scraped_img`-1 WHERE `id`='".$row->website."'");
			logger($row->idkeywords, $row->original, 'Build failed: no snippets', $this->link);
		} else {
		if(count(explode(" ", $serpcontent)) > 201){
			if(empty($filename)) {
				...
				mysqli_query($this->link, "UPDATE `keywords` SET `built`='2',`reason`='There was an error with image download/ftp transfer.',`last_action`='builder', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
				mysqli_query($this->link, "UPDATE `websites` SET `scraped`=`scraped`-1,`scraped_img`=`scraped_img`-1 WHERE `id`='".$row->website."'");
				logger($row->idkeywords, $row->original, 'Build failed: error image download/ftp transfer', $this->link);
			} else {
				...
				if(empty($wordpress[$row->website])){
					...
					if($conn1 === false){
						...
						mysqli_query($this->link, "UPDATE `keywords` SET `built`='2',`reason`='ERROR: Could not connect to remote database.',`last_action`='builder', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
						mysqli_query($this->link, "UPDATE `websites` SET `scraped`=`scraped`-1,`scraped_img`=`scraped_img`-1 WHERE `id`='".$row->website."'");
						logger($row->idkeywords, $row->original, 'Build failed: can\'t connect to remote db', $this->link);
					} else {
						...
						if(mysqli_num_rows($exists) < 1){
							...
							if($ins){
								if($total_built[$website_id->id] >= $preload[$website_id->id] && $preload[$website_id->id] != 0){
									end_preload($ftp[$website_id->id], $ftpuser[$website_id->id], $ftppassword[$website_id->id]);
								...
								mysqli_query($this->link, "INSERT INTO `built`(`idkeywords`,`website`,`author`,`title`,`content`,`excerpt`,`keywordtop1`,`keywordtop2`,`keywordtop3`,`keywordtop4`,`keywordtop5`,`keywordtop6`,`keywordtop7`,`keywordtop8`,`keywordtop9`,`keywordtop10`,`keywordtop11`,`keywordtop12`,`keywordtop13`,`keywordtop14`,`keywordtop15`,`keywordtop16`) VALUES('".$row->idkeywords."', '".$row->website."', '".$authorid."', '".mysqli_real_escape_string($this->link, $ptitle)."', '".mysqli_real_escape_string($this->link, $content)."', '".mysqli_real_escape_string($this->link, $excerpt)."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop1))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop2))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop3))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop4))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop5))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop6))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop7))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop8))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop9))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop10))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop11))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop12))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop13))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop14))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop15))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop16))."')");
								mysqli_query($this->link, "UPDATE `keywords` SET `built`='1',`reason`='',`last_action`='builder success', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
								mysqli_query($this->link, "UPDATE `websites` SET `built`=`built`+1,`total_built`=`total_built`+1 WHERE `id`='".$row->website."'");
								logger($row->idkeywords, $row->original, 'Build success.', $this->link);
							} else {
								//if fail to remote
								...
								mysqli_query($this->link, "UPDATE `keywords` SET `built`='2',`reason`='Failed to add post to remote database. It doesn\'t exist, error at inserting.',`last_action`='builder', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
								mysqli_query($this->link, "UPDATE `websites` SET `scraped`=`scraped`-1,`scraped_img`=`scraped_img`-1 WHERE `id`='".$row->website."'");
								logger($row->idkeywords, $row->original, 'Build failed: can\'t add to remote db, it doesn\'t exist there', $this->link);
							}
						} else {
							//if exists at remote
							echo "KEYWORD ID: ".$row->idkeywords." FAILED WITH STATUS: Post already exists into remote table.<br>";
							mysqli_query($this->link, "UPDATE `keywords` SET `built`='2',`reason`='Post already exists into remote table.',`last_action`='builder', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
							mysqli_query($this->link, "UPDATE `websites` SET `scraped`=`scraped`-1,`scraped_img`=`scraped_img`-1 WHERE `id`='".$row->website."'");
							logger($row->idkeywords, $row->original, 'Build failed: it exists into remote db', $this->link);
						}
					...
					if($add == '1'){
							...
							mysqli_query($this->link, "INSERT INTO `built`(`idkeywords`,`website`,`author`,`title`,`content`,`excerpt`,`keywordtop1`,`keywordtop2`,`keywordtop3`,`keywordtop4`,`keywordtop5`,`keywordtop6`,`keywordtop7`,`keywordtop8`,`keywordtop9`,`keywordtop10`,`keywordtop11`,`keywordtop12`,`keywordtop13`,`keywordtop14`,`keywordtop15`,`keywordtop16`) VALUES('".$row->idkeywords."', '".$row->website."', '".$authorid."', '".mysqli_real_escape_string($this->link, $ptitle)."', '".mysqli_real_escape_string($this->link, $content)."', '".mysqli_real_escape_string($this->link, $excerpt)."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop1))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop2))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop3))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop4))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop5))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop6))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop7))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop8))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop9))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop10))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop11))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop12))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop13))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop14))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop15))."', '".mysqli_real_escape_string($this->link, url_slug($row->keywordtop16))."')");
							mysqli_query($this->link, "UPDATE `keywords` SET `built`='1',`reason`='',`last_action`='builder success', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
							mysqli_query($this->link, "UPDATE `websites` SET `built`=`built`+1 WHERE `id`='".$row->website."'");
							logger($row->idkeywords, $row->original, 'Build success wordpress.', $this->link);
						} else {
							//if fail to remote
							echo "KEYWORD ID: ".$row->idkeywords." FAILED WITH STATUS: ".$add."<br>";
							mysqli_query($this->link, "UPDATE `keywords` SET `built`='2',`reason`='Failed to add post to wordpress.',`last_action`='builder', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
							mysqli_query($this->link, "UPDATE `websites` SET `scraped`=`scraped`-1,`scraped_img`=`scraped_img`-1 WHERE `id`='".$row->website."'");
							logger($row->idkeywords, $row->original, 'Build failed wordpress: '.$add, $this->link);
						}					
				}
			}
		} else {
			echo "KEYWORD ID: ".$row->idkeywords." FAILED WITH STATUS: Serp content under 200 words.<br>";
			mysqli_query($this->link, "UPDATE `keywords` SET `built`='2',`reason`='Serp content under 200 words.',`last_action`='builder', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE `idkeywords`='".$row->idkeywords."'");
			mysqli_query($this->link, "UPDATE `websites` SET `scraped`=`scraped`-1,`scraped_img`=`scraped_img`-1 WHERE `id`='".$row->website."'");
			logger($row->idkeywords, $row->original, 'Build failed: serp content under 200 words', $this->link);
		}
		}
		}
	}
...
function find_and_replace($original_text, $find_and_replace_website){

	
	$select = mysqli_query($this->link, "SELECT * FROM `config_global` WHERE `name`='FIND_AND_REPLACE'") or die(mysqli_error($this->link));
...
function prefixes($ptitle){
	
	$prefix_found = 0;
	
	$select = mysqli_query($this->link, "SELECT * FROM `config_global`") or die(mysqli_error($this->link));
	if(mysqli_num_rows($select) > 0){
		while($row = mysqli_fetch_object($select)){
...
function ignore_url($url){
	...
	$select = mysqli_query($this->link, "SELECT * FROM `config_global` WHERE `name`='IMAGE_DOWNLOAD_IGNORE'") or die(mysqli_error($this->link));
	if(mysqli_num_rows($select) > 0){
		while($row = mysqli_fetch_object($select)){
				$look = explode("\r\n", $row->value);
				foreach($look as $pm){
					if(preg_match("/".$pm."/i", $this->domainmatch($url))){
						$blacklist_found = 1;
					}
				}
		}
