...
<?php
$select = mysql_query("SELECT id FROM `topics` WHERE published='2' ORDER BY id DESC") or die(mysql_error());
...
$select1 = mysql_query("SELECT id,id1,published,title,answers,scraper,url FROM `topics` WHERE published='2' ORDER BY id DESC LIMIT ".$first.",".$last."") or die(mysql_error());
...
?>		
		<tr<?php echo $class; ?>>
			...
			<td><a href="topicsspam.php?delete=<?php echo $row->id; ?>">Delete</td>
		</tr>
<?php
}
...
if($page > 1)
	{
		echo "&nbsp;<a href='topicsspam.php?page=1'>First</a>";
	}
	...
	for ($i = ($page - 2); $i <= ($page + 2); $i++)
	{
		if($i > 0 && $i <= $seiten)
		{
			...
			{
			echo "&nbsp;-&nbsp;<a href=topicsspam.php?page=".$i.">";
				echo "".$i."";
			}
			echo "</a>";
		}
	...
	{
		echo "&nbsp;-&nbsp;<a href=topicsspam.php?page=".$seiten.">Last</a>";
	}
