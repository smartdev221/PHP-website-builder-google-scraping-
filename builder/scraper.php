	...
	///
		$getconf = mysqli_query($conn, "SELECT name,value_ FROM `config`");
			while($getconfig = mysqli_fetch_object($getconf)){
				...
				}elseif($getconfig->name == "PROXY_IP_PORT_SMART"){
					$PROXY_IP_PORT_SMART = $getconfig->value_;
				}elseif($getconfig->name == "PROXY_USER_PASSWORD_SMART"){
					$PROXY_USER_PASSWORD_SMART = $getconfig->value_;
...
sleep($rand1);
	...
	if($running_threads<$MAX_THREADS){
		///
		$website_check = mysqli_query($conn, "SELECT `id` FROM `websites` WHERE `scraped`<`dripfeed_current` AND `done_today`='0' AND `skip`='0' LIMIT 1");
		///
		...
		$sel = mysqli_query($conn, "SELECT idkeywords,website,original,related1,related2,related3,related4,related5,related6,related7,related8,word2,word3,result1,result2,result3,result4,result5,result6,result7,result8,result9,result10,scraped FROM keywords WHERE scraped=0 and `website`='".$website_id->id."' LIMIT ".$MAX_KEYWORDS_X_THREAD." ");
			$rownum = mysqli_num_rows($sel);
			
			while($row = mysqli_fetch_object($sel)){
				if($PAUSED==0){
				mysqli_query($conn, "UPDATE `keywords` SET `scraped`='".$rand."',`last_action`='g_scraper', `last_action_date`='".date("Y-m-d H:i:s")."' WHERE idkeywords='".$row->idkeywords."'");
				}
			}
			sleep(1);
		...
		while($k<$rownum){
			...
			$sel1 = mysqli_query($conn, "SELECT idkeywords,website,original,related1,related2,related3,related4,related5,related6,related7,related8,word2,word3,result1,result2,result3,result4,result5,result6,result7,result8,result9,result10,scraped FROM keywords WHERE scraped='".$rand."' and `website`='".$website_id->id."' LIMIT 1");
			...
			$asd = mysqli_query($conn, "UPDATE `keywords` SET `scraped`='1' WHERE idkeywords='".$kw->idkeywords."'") or die(mysqli_error($conn));
			if($asd){
				echo "updatat\n";
			} else {
				echo "failed\n";
			...
			if(!empty($kw->original)){
				logger($kw->idkeywords, $kw->original, 'Google scraping', $conn);
			}
			//proxy login details
			$kw->proxy_ip_port = $PROXY_IP_PORT_SMART;
			$kw->proxy_user_pass = $PROXY_USER_PASSWORD_SMART;
