...
sleep($rand1);
	...
	if($running_threads<$MAX_THREADS){
		///
		$website_check = mysqli_query($conn, "SELECT `id` FROM `websites` WHERE `scraped_vid`<`perday` AND `scraped`>=`perday` AND `done_today`='0' AND `skip`='0' LIMIT 1");
		///
		...
		$sel = mysqli_query($conn, "SELECT idkeywords,website,original,related1,related2,related3,related4,related5,related6,related7,related8,word2,word3,result1,result2,result3,result4,result5,result6,result7,result8,result9,result10,scraped FROM keywords WHERE video=0 and scraped=1 and `result1`!='' and `website`='".$website_id->id."' LIMIT ".$MAX_KEYWORDS_X_THREAD." ");
			$rownum = mysqli_num_rows($sel);
			
			while($row = mysqli_fetch_object($sel)){
				if($PAUSED==0){
				mysqli_query($conn, "UPDATE `keywords` SET `video`='".$rand."' WHERE idkeywords='".$row->idkeywords."'");
				}
			}
			sleep(1);
		...
		while($k<$rownum){
			...
			$sel1 = mysqli_query($conn, "SELECT idkeywords,website,original,related1,related2,related3,related4,related5,related6,related7,related8,word2,word3,result1,result2,result3,result4,result5,result6,result7,result8,result9,result10,scraped FROM keywords WHERE video='".$rand."' and `website`='".$website_id->id."' LIMIT 1");
			...
			$asd = mysqli_query($conn, "UPDATE `keywords` SET `video`='1' WHERE idkeywords='".$kw->idkeywords."'") or die(mysqli_error($conn));
			if($asd){
				echo "updatat\n";
			} else {
				echo "failed\n";
