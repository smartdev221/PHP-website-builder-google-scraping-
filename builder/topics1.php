...
<div class="widget_heading">
  <h4>Topics for the search term "<?php echo mysql_real_escape_string($_GET['search']); ?>"</h4>
</div>
...
<?php
$select = mysql_query("SELECT id,id1,published,title,answers,scraper,url FROM `topics` WHERE `title` LIKE '%".mysql_real_escape_string($_GET['search'])."%' ORDER BY id DESC") or die(mysql_error());
...
?>		
		<tr<?php echo $class; ?>>
			...
			<td><a href="topics1.php?delete=<?php echo $row->id; ?>&search=<?php echo $_GET['search']; ?>">Delete</a></td>
		</tr>
<?php
}
}
