...
<?php
$select = mysql_query("SELECT idkeywords FROM `keywords` WHERE `scraped`='1' and `result1`!=''") or die(mysql_error());
...
$select1 = mysql_query("SELECT idkeywords,original,scraped,video,images,url_scraped,translated_qa,translated_snippet,built FROM `keywords` WHERE `scraped`='1' and `result1`!='' ORDER BY `idkeywords` DESC LIMIT ".$first.",".$last."") or die(mysql_error());
...
?>		
		<tr<?php echo $class; ?>>
			...
			<?php if($row->built == '2'){ ?>
			<td><a href="keywords_failed.php" title="click for reason"><?php echo $row->original; ?></td>
			<?php } else { ?>
			<td><?php echo $row->original; ?></td>
			<?php } ?>
			<td><?php echo yesno($row->scraped,$row->idkeywords, 1);  ?></td>
			...
			<td><?php echo yesno($row->translated_qa,$row->idkeywords, 5); ?></td>
			<td><?php echo yesno($row->translated_snippet,$row->idkeywords, 6); ?></td>
...
if($page > 1)
	{
		echo "&nbsp;<a href='keywords_scraped.php?page=1'>First</a>";
	}
	...
	for ($i = ($page - 2); $i <= ($page + 2); $i++)
	{
		if($i > 0 && $i <= $seiten)
		{
			...
			{
			echo "&nbsp;-&nbsp;<a href=keywords_scraped.php?page=".$i.">";
				echo "".$i."";
			}
			echo "</a>";
		}
	...
	{
		echo "&nbsp;-&nbsp;<a href=keywords_scraped.php?page=".$seiten.">Last</a>";
	}
