...
<div class="row">
  ...
  <div class="span6"><div class="widget_heading"><h4>Select Export method and click Export</h4></div><div class="widget_container">
  ...
  if(!empty($_POST['topost'])){
	...
	echo '<font color="green" size="3">Please do not close this page until it finishes loading.</font><br>';
		...
		if($_POST['method'] == "1"){
			...
			$mysqll = mysql_query("SELECT `title` FROM `topics` WHERE `published`='1'") or die(mysql_error());
				while($row = mysql_fetch_object($mysqll)){
					$txt = replaces1($row->title);
					$text = array($txt);
					//$txt = $row->title."\n";
					fputcsv($myfile, $text, ',');
					//echo $row->title."<br>";
			...
			echo '<font color="green" size="3">------------------------------------------------<br>Click <a href="export.csv" target="_blank">here to download the export</a></font><br><br>';
			...
			echo '<font color="green" size="3">------------------------------------------------<br>Click <a href="export.txt" target="_blank">here to see the export</a></font><br><br>';
  ...
  <div class="controls">
	<select name="method" class="input-xlarge span5">
		<option value="1">to CSV</option>
		<option value="2">to TXT</option>
	</select>
